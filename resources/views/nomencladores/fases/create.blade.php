...
@section('content')
      <div class="card">
        <h4 class="card-header"> {{ trans('aplicacion.c_fase') }}</h4>
        <div class="card-body">
           ...
           {{-- Esto es para mostrar listado de errores al comienzo --}}
            ...
            <form method="POST" action="{{ route('fases.crear') }}">
                {{ csrf_field() }}
                ...
                <button type="submit" class="btn btn-primary">{{ trans('aplicacion.c_fase') }}</button>
