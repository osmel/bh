...
@section('content')
      <div class="card">
        <h4 class="card-header"> {{ trans('aplicacion.c_tipo_referencia') }}</h4>
        <div class="card-body">
           ...
           {{-- Esto es para mostrar listado de errores al comienzo --}}
            ...
            <form method="POST" action="{{ route('tipo_referencias.crear') }}">
                {{ csrf_field() }}
                ...
                <button type="submit" class="btn btn-primary">{{ trans('aplicacion.c_tipo_referencia') }}</button>
